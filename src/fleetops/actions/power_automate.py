...
incident / notifies the route controller. Gated on approval.require_approved —
...
The trigger URL is a secret: stored in Key Vault (kv-fleetops-*, secret name
...
from fleetops.common.config import get_settings
...
def _trigger_url() -> str:
    ...
    if not settings.key_vault_uri:
        raise RuntimeError("FLEETOPS_KEY_VAULT_URI is not set — see .env.example.")
...
def create_maintenance_incident(approval_id: str, vehicle_id: str, route_id: str, summary: str) -> dict[str, Any]:
    ...
    RecordStore("actions").create({
        "actionType": "power_automate.create_maintenance_incident",
        "approvalId": approval_id,
        "payload": payload,
        "result": result,
    })
